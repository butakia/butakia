...
import { Film, PenLine, Menu, X } from "lucide-react";
...
const CATEGORIES = [
  { label: "Inicio", href: "/libros" },
  { label: "Cuentos", href: "/libros/genero/Cuentos" },
  { label: "Novela", href: "/libros/genero/Novela" },
  { label: "Poesía", href: "/libros/genero/Poesía" },
  { label: "Ensayo", href: "/libros/genero/Ensayo" },
  { label: "Terror", href: "/libros/genero/Terror" },
  { label: "Ciencia Ficción", href: "/libros/genero/Ciencia Ficción" },
  { label: "Dominio Público", href: "/libros/genero/Dominio Público" },
  { label: "Foro", href: "/libros/foro" },
];

export default function LibrosHeader({ tagline = "Libros" }: { tagline?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 sm:gap-4">
        <Link
          href="/"
          className="hidden items-center gap-1.5 rounded-full border border-white/15 px-3.5 py-1.5 text-sm font-semibold text-white/70 transition-colors hover:bg-white/10 sm:flex"
        >
          <Film size={15} />
          Ver películas
        </Link>
        <Link
          href="/libros/publicar"
      ...
      {mobileMenuOpen && (
        <nav className="absolute inset-x-0 top-full flex flex-col border-b border-border bg-background/98 p-4 backdrop-blur-md transition-all duration-150 lg:hidden">
          ...
          <Link
            href="/"
            className="mt-2 flex items-center justify-center gap-1.5 rounded-lg border border-white/15 px-3.5 py-2.5 text-sm font-semibold text-white/70"
          >
            <Film size={15} />
            Ver películas
          </Link>
