...
export default function BookHeaderSearch() {
  ...
  return (
    <div ref={rootRef} className="relative">
      {open ? (
        <div className="absolute right-0 top-1/2 z-50 flex w-[85vw] max-w-[320px] -translate-y-1/2 items-center gap-2 rounded-full border border-white/15 bg-black/90 px-3 py-2 backdrop-blur-md transition-all duration-200 sm:w-[280px]">
          ...
          />
          <button aria-label="Cerrar búsqueda" onClick={close} className="shrink-0 text-white/50 transition-colors hover:text-white">
            <X size={16} />
          </button>
        </div>
      ) : (
      ...
      {open && q && (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 max-h-96 w-80 max-w-[85vw] overflow-y-auto rounded-xl border border-white/10 bg-black/70 shadow-2xl backdrop-blur-2xl">
          {results.length === 0 ? (
            <p className="p-4 text-sm text-white/50">Sin resultados para &quot;{query}&quot;.</p>
          ) : (
